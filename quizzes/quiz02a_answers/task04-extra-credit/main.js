...
function artistMatch(track, searchTerm) {
    return track.artist.name.toLowerCase().includes(searchTerm.toLowerCase());
}

function trackMatch(track, searchTerm) {
    return track.name.toLowerCase().includes(searchTerm.toLowerCase());
}

function albumMatch(track, searchTerm) {
    return track.album.name.toLowerCase().includes(searchTerm.toLowerCase());
}

function isMatch(track, searchTerm) {
    return (
        artistMatch(track, searchTerm) ||
        trackMatch(track, searchTerm) ||
        albumMatch(track, searchTerm)
    );
}
...
function showMatchingTracks() {
    ...
    for (const track of tracks) {
        if (!isMatch(track, searchTerm)) {
            continue;
        }
        const template = ` <section class="track">
